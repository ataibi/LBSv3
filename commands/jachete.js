...
module.exports.run = async (bot, message, args) => {
  ...
  con.query(`SELECT * FROM experience WHERE id = '${author.id}' AND guild = '${message.guild.id}'`, (e, certi) => {
    ...
    if (!certi[0]) { certitude = 0 } else { certitude = certi[0].xp }
    let levelPool = 1234
    ...
    con.query(`SELECT * FROM certithunes WHERE id = '${author.id}' AND guild = '${message.guild.id}'`, (err, row) => {
      ...
      if (!row[0]) { certithunes = 0 } else { certithunes = row[0].amount }
      const itemId = args[0]
      ...
      if (certithunes < stock[itemId].price) {
        ...
        const now = new Date()
        let limit
        if (itemId === 1) { limit = new Date().setDate(now.getDate() + 3) } else if (itemId === 2) {
          limit = new Date().setDate(now.getDate() + 7)
          if (typeof args[1] !== 'string' || args[1].length !== 6 || isNaN(parseInt(args[1], 16))) { return message.reply("ton code couleur est pas bon, c'est sous la forme aabbcc") } else {
            message.guild.createRole({
              ...
              .then(role => {
                const rolePosition = message.guild.roles.size - 2
        ...
        con.query(`SELECT * FROM shop WHERE userID = '${message.author.id}' AND guildID = '${message.guild.id}' AND itemID = '${itemId}'`, (error, items) => {
          ...
          const sql = (`UPDATE certithunes SET amount = amount - ${stock[itemId].price} WHERE id = '${message.author.id}' AND guild = '${message.guild.id}'`)
          const msg = `tu viens d'acheter 1 ${stock[itemId].title}`
          if (!items[0]) {
